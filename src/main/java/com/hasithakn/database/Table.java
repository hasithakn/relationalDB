...
public class Table
    ...
    public Table project(String attributes) {
        ...
        int[] columnNos = match(attrs);

        tuples.forEach(e -> {
            Comparable[] temp = new Comparable[columnNos.length];
            for (int i = 0; i < columnNos.length; i++) {
                temp[i] = e[columnNos[i]];
            }
            rows.add(temp);
        });
    ...
    public Table union(Table table2) {
        ...
        int[] match = match(key);
        List<Comparable[]> unionTuples = table2.tuples;
        a:
        for (Comparable[] unionTuple : unionTuples) {
            boolean shouldAdd = true;
            c:
            for (Comparable[] tuple : tuples) {
                b:
                for (int i : match) {
                    if (tuple[match[i]].equals(unionTuple[match[i]])) {
                        shouldAdd = false;
                        break b;
                    }
                }
